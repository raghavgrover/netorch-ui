...
const Inventory = (() => {
    ...
    function downloadTemplate() {
        const content = [
            '# =============================================================================',
            '# netorch inventory template — Ansible-compatible INI format',
            '#',
            '# HOW TO USE:',
            '#   1. Fill in your device IPs, platforms, and credentials below.',
            '#   2. Remove or comment out any sections/hosts you do not need.',
            '#   3. Upload this file via Inventory → Upload File.',
            '#',
            '# SUPPORTED PLATFORMS:',
            '#   cisco_ios   — Cisco IOS / IOS-XE (Catalyst, ASR, ISR)',
            '#   cisco_xe    — Cisco IOS-XE (alias for cisco_ios)',
            '#   cisco_xr    — Cisco IOS-XR (NCS, ASR 9K) — requires commit',
            '#   cisco_nxos  — Cisco NX-OS (Nexus switches)',
            '#   juniper_junos — Juniper Junos (MX, EX, SRX)',
            '#   fortinet    — Fortinet FortiOS (FortiGate)',
            '#   linux       — Linux hosts (Ubuntu, RHEL, Debian, etc.)',
            '#',
            '# CREDENTIAL LOOKUP ORDER (per device):',
            '#   1. Inline credentials on the host line (highest priority)',
            '#   2. Group-level [group:vars] defaults',
            '#   3. Global [all:vars] defaults',
            '#   4. Vault (OpenBao / CyberArk) if password is blank',
            '# =============================================================================',
            '',
            '# ---------------------------------------------------------------------------',
            '# Global defaults applied to every host unless overridden',
            '# ---------------------------------------------------------------------------',
            '[all:vars]',
            'port = 22',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 1: Cisco IOS/IOS-XE — full inline credentials',
            '# ---------------------------------------------------------------------------',
            '[cisco_switches]',
            '192.168.1.10  platform=cisco_ios  username=netaudit  password=MySecret  enable_secret=MyEnable  port=22',
            '192.168.1.11  platform=cisco_xe   username=netaudit  password=MySecret  enable_secret=MyEnable',
            '192.168.1.12  platform=cisco_ios  username=netaudit  password=MySecret',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 2: Cisco IOS-XR — commit is handled automatically',
            '# ---------------------------------------------------------------------------',
            '[core_routers]',
            '10.0.0.1  platform=cisco_xr  username=xruser  password=XRSecret',
            '10.0.0.2  platform=cisco_xr  username=xruser  password=XRSecret',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 3: Cisco NX-OS',
            '# ---------------------------------------------------------------------------',
            '[nexus_fabric]',
            '10.1.0.10  platform=cisco_nxos  username=admin  password=NXSecret',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 4: Juniper Junos',
            '# ---------------------------------------------------------------------------',
            '[juniper_edge]',
            '172.16.0.1  platform=juniper_junos  username=netops  password=JuniperPass',
            '172.16.0.2  platform=juniper_junos  username=netops  password=JuniperPass',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 5: Fortinet FortiGate',
            '# ---------------------------------------------------------------------------',
            '[firewalls]',
            '10.10.0.1  platform=fortinet  username=admin  password=FGPass  port=22',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 6: Linux servers',
            '# ---------------------------------------------------------------------------',
            '[linux_servers]',
            '10.20.0.10  platform=linux  username=sysadmin  password=LinuxPass  port=22',
            '10.20.0.11  platform=linux  username=sysadmin  password=LinuxPass',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 7: Username only — password fetched from Vault at runtime',
            '# ---------------------------------------------------------------------------',
            '[vault_managed]',
            '10.30.0.1  platform=cisco_ios  username=netaudit',
            '10.30.0.2  platform=linux      username=sysadmin',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 8: No inline credentials — all resolved from Vault',
            '# (host IP is the Vault lookup key)',
            '# ---------------------------------------------------------------------------',
            '[fully_vault_managed]',
            '10.40.0.1  platform=cisco_xr',
            '10.40.0.2  platform=juniper_junos',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 9: Group-level vars — shared credentials for the whole group',
            '# ---------------------------------------------------------------------------',
            '[group_defaults:vars]',
            'username = groupuser',
            'password = GroupSecret',
            'enable_secret = GroupEnable',
            '',
            '[group_defaults]',
            '192.168.10.1  platform=cisco_ios',
            '192.168.10.2  platform=cisco_ios',
            '192.168.10.3  platform=cisco_xe',
            '',
            '',
            '# ---------------------------------------------------------------------------',
            '# Example 10: Non-standard SSH port',
            '# ---------------------------------------------------------------------------',
            '[non_standard_ports]',
            '10.50.0.1  platform=linux  username=admin  password=Pass  port=2222',
            '10.50.0.2  platform=cisco_ios  username=netaudit  password=Pass  port=830',
        ].join('\n');
        ...
        a.href     = url;
        a.download = 'netorch_inventory_template.ini';
